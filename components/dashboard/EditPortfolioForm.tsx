...
const EditPortfolioForm = ({ portfolio: initialPortfolio }) => {
  ...
  useEffect(() => {
    if (initialPortfolio) {
      setForm({
        ...
        department: initialPortfolio.department,
        description: initialPortfolio.description,
        category: initialPortfolio.category,
      });
    }
  }, [initialPortfolio]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let imageUrl = initialPortfolio.image;
    if (image) {
    ...
    try {
      const res = await fetch(`/api/portfolio?id=${initialPortfolio._id}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        alert(`Error updating portfolio item: ${errorData.message}`);
      }
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Fill out the form below to edit the portfolio item.</CardDescription>
      </CardHeader>
        ...
        <CardContent>
          <div className="grid w-full items-center gap-4">
            ...
            <div className="flex flex-col space-y-1.5">
              ...
              <Input id="department" placeholder="Enter the department" value={form.department} onChange={handleChange} />
            </div>
            <div className="flex flex-col space-y-1.5">
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" placeholder="Enter the description" value={form.description} onChange={handleChange} />
            </div>
            <div className="flex flex-col space-y-1.5">
