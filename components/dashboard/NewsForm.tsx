...
const NewsForm = () => {
  ...
  const handleGeneratePost = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/generate-news-content", { // Placeholder webhook URL
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ title: form.title, subTitle: form.subTitle }),
      });
      ...
      if (res.ok) {
        ...
        setForm((prevForm) => ({
          ...
          // Assuming the webhook returns an imageUrl
        }));
        // If the webhook returns an image URL, you might want to handle it differently
        // For now, let's assume it updates the 'post' content.
        // If it returns an image URL, you'd need to set it to the 'image' state or a new 'imageUrl' state
        // setImage(data.imageUrl); // Uncomment if webhook returns image URL
      } else {
        const errorData = await res.json();
        alert(`Error generating post: ${errorData.message}`);
      }
    } catch (error) {
      console.error("Error generating post:", error);
      alert("An unexpected error occurred while generating the post. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        alert(`Error creating news post: ${errorData.message}`);
      }
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Fill out the form below to create a new news post.</CardDescription>
      </CardHeader>
